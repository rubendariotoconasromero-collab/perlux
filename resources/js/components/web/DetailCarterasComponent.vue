  ...
  <div>
            ...
            <div class="product-image-wrapper-main">
              ...
              <div class="heart-wishlist" @click="toggleWishlist">
                <i :class="isFavorite ? 'fas fa-heart' : 'far fa-heart'"
                  :style="{ color: isFavorite ? '#7e7e7e' : '#ccc' }"></i>
              </div>
            </div>
    ...
    <div class="third-section-container container-fluid py-5 bg-light-custom">
      <div class="text-center mb-5 content-logo-glam">
        <img src="/images/site/resources/logo_perlux.svg" alt="Perlux" class="glam-home-image img-fluid" style="max-width: 140px;">
        <h3 class="section-title-big mt-3">PRODUCTOS RELACIONADOS</h3>
      </div>
        ...
        <div class="slider-container">
          ...
          <div class="slider-wrapper" ref="productSlider">
            <div class="slider-track">
              ...
              <div class="slider-item" v-for="(item, index) in relatedProducts" :key="item.ProductID">
                <div class="card product-card h-100 border-0 bg-white rounded-0">
                  <div class="card-body p-0 d-flex flex-column h-100">
                    
                    <div class="product-image-container">
                      <a :href="'/detail_carteras?product_id=' + item.ProductID">
                    ...
                    <div class="py-3 d-flex flex-column flex-grow-1 fondo-info-product">
                      <div class="d-flex justify-content-between align-items-start mb-3">
                        ...
                        <button class="btn heart-btn-bottom p-0 border-0" @click="toggleFavoriteList(item)"
                          ...
                          <i :class="isProductFavorite(item) ? 'fas fa-heart text-secondary-custom' : 'far fa-heart'" style="font-size: 1.3rem;"></i>
                        </button>
                      </div>
            ...
            <div class="col-md-6 offset-md-1">
              <div class="rating-bars">
                <div v-for="star in [5, 4, 3, 2, 1]" :key="star" class="d-flex align-items-center mb-2">
                  <span class="star-label me-2">{{ star }} <i class="fas fa-star" style="font-size: 0.7rem;"></i></span>
        ...
        <div class="reviews-list mt-5">
          ...
          <div v-else>
            <div v-if="reviews.length === 0" class="text-center text-muted py-5">
              ...
              <p>Sé la primera persona en calificar esta cartera.</p>
            </div>
      ...
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content rounded-0 border-0">
          ...
          <div class="modal-body text-center p-4 p-md-5 pt-2">
            <h3 class="fw-bold text-uppercase mb-3">GUÍA DE REFERENCIA</h3>
            <p class="text-muted mb-4 small">
              Información de dimensiones y cuidados para nuestras carteras Perlux.
            </p>
            <div class="table-responsive d-flex justify-content-center">
               <p>Información específica de carteras aquí.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Modals (Calificacion, Auth) as in DetailBodyComponent -->
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      tabs: [ { id: 'details', title: 'Detalles' }, { id: 'shipping', title: 'Compras y devoluciones' } ]
    };
  },
  computed: {
    ...
    relatedProducts() { 
        return this.products.filter(item => item.Tipo === 'Carteras' && item.ProductID !== this.product.ProductID); 
    },
  ...
  methods: {
    ...
    pedirPorWhatsapp(product) {
        ...
        let message = `Hola Perlux, estoy interesada en la cartera: *${product.Name}*`;
    ...
    getPlaceholderImage(i) { return `https://via.placeholder.com/300x400?text=Cartera+${i}`; },
    ...
    updateCarrito() {
        ...
        if (!this.selectedSize) { Swal.fire({ icon: 'warning', title: 'Elige una opción', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 }); return; }
    ...
    comprar(item) { window.location.href = `/detail_carteras?product_id=${item.ProductID}`; },
    ...
    async submitReview() { /* ... similar to DetailBody ... */ },
  },
  async mounted() {
    this.refreshCartData();
...
.indicator { width: 8px; height: 8px; border-radius: 50%; background: #ccc; cursor: pointer; transition: all 0.3s ease; opacity: 1; }
.indicator.active { background: #7c7c7c; transform: scale(1.2); }
...
.tab-button {
  ...
  border-bottom: 2px solid transparent; margin-bottom: -2px; transition: color 0.1s; width: 100%;
}
...
.btn-comprar:hover { background: #969696; color: #fff; border-color: #969696; }
...
.btn-whatsapp-card {
  ...
  text-transform: uppercase; transition: all 0.1s ease; margin-top: 8px;
  display: flex; align-items: center; justify-content: center;
}
...
.rating-number { font-size: 3.5rem; font-weight: 300; line-height: 1; }
.rating-max { font-size: 1.5rem; color: #999; }
.border-light-gray { border-color: #000 !important; border-bottom: 1px solid #000 !important; }

/* Mobile Thumbnails Slider */
.product-thumbnails-mobile {
  width: 100%;
  overflow: hidden;
  margin-top: 20px !important; /* Baja la sección de miniaturas */
}
