<template>
    ...
    <div class="container-fluid">
      ...
      <nav class="navbar navbar-expand-lg p-0">
        ...
        <a href="/" class="navbar-brand order-2 order-lg-1">
          <img src="/images/site/svg/skinly_logo.svg" alt="Perlux Logo" class="img-fluid" loading="lazy" />
        </a>
          ...
          <ul class="navbar-nav mx-auto">
            ...
            <li class="nav-item dropdown" @mouseenter="showDropdown('jeans')" @mouseleave="hideDropdown('jeans')">
              ...
              <div class="dropdown-menu collections-dropdown" :class="{ show: activeDropdown === 'jeans' }">
                ...
                <hr>
                <!-- </template> -->
                <div class="dropdown-content dropdown-content-jeans">
                  <div class="collections-section">
                    <h5 class="dropdown-title">COLECCIONES</h5>
                    <div class="collections-list">
        ...
        <div v-for="product in favorites" :key="product.ProductID" class="favorite-item">
          ...
          <div class="favorite-info">
            ...
            <div class="product-colors">
              <span v-for="color in product.colors.slice(0, 3)" :key="color.ColorID" class="color-dot"
                :style="{ backgroundColor: color.HexCode }"></span>
            </div>
          </div>
  ...
  <div class="third-section-container container-fluid">
    <div class="third-section text-center">
      <div class="third-section-title">
        <div class="row h-100 mx-0">
          <div class="col-md-5 my-auto col-title1">
            <h2 class="title1"><span>¿CANSADA</span><br>de las miradas?</h2>
          </div>
          <div class="col-md-2 mx-auto my-auto">
            <!-- <a href="#" class="btn btn-outline-light btn-lista">Hazlo tuyo</a> -->
          </div>
          <div class="col-md-5 my-auto col-title2">
            <h2 class="title2"><span>¡PIDE YA</span><br>tu segunda piel!</h2>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="fourth-section">
      <div class="fourth-section-filters d-flex flex-wrap">

        <div class="col-lg-3 col-12 content-filter d-flex flex-column">
          <div class="form-group filter-group1 w-100">
            <div class="d-flex justify-content-between align-items-center">
              <label class="title-filter" for="">DISPONIBILIDAD</label>
            </div>

            <div class="mb-3 position-relative w-100" style="border: 1px solid #000;">
              <div class="dropdown dropdown-select" style="padding-right:12px;">
                <button class="btn dropdown-toggle form-select-filter" type="button" data-bs-toggle="dropdown"
                  aria-expanded="false" id="ordenarDropdown">
                  {{ selectedFilterText || 'Selecciona' }}
                </button>
                <ul class="dropdown-menu w-100" aria-labelledby="ordenarDropdown">
                  <li>
                    <a class="dropdown-item dropdown-item-select" href="#" data-value=""
                      @click.prevent="filterProducts('')">
                      Lo más nuevo primero
                    </a>
                  </li>
                  <li>
                    <a class="dropdown-item dropdown-item-select" href="#" data-value="2"
                      @click.prevent="filterProducts('2')">
                      Lo más viejo primero
                    </a>
                  </li>
                  <li>
                    <a class="dropdown-item dropdown-item-select" href="#" data-value="3"
                      @click.prevent="filterProducts('3')">
                      Alfabéticamente A-Z
                    </a>
                  </li>
                  <li>
                    <a class="dropdown-item dropdown-item-select" href="#" data-value="4"
                      @click.prevent="filterProducts('4')">
                      Alfabéticamente Z-A
                    </a>
                  </li>
                  <li>
                    <a class="dropdown-item dropdown-item-select" href="#" data-value="5"
                      @click.prevent="filterProducts('5')">
                      Precio de menor a mayor
                    </a>
                  </li>
                  <li>
                    <a class="dropdown-item dropdown-item-select" href="#" data-value="6"
                      @click.prevent="filterProducts('6')">
                      Precio de mayor a menor
                    </a>
                  </li>
                  <li>
                    <a class="dropdown-item dropdown-item-select" href="#" data-value="7"
                      @click.prevent="filterProducts('7')">
                      Destacado
                    </a>
                  </li>
                </ul>
              </div>
              <i class="fas fa-bars text-muted position-absolute"
                style="top: 50%; right: 12px; transform: translateY(-50%); pointer-events: none;"></i>
            </div>
          </div>

          <div class="form-group filter-group2">
            ...
            <transition name="fade">
              <div v-if="showSections.availability" class="mt-3">
                <div class="d-flex justify-content-start align-items-center check-content">
                  <input type="checkbox" class="check-filter" :checked="filters.availability.includes('inStock')"
                    @change="updateAvailabilityFilter('inStock', $event)" />
                  <p class="check-label">En Stock</p>
                </div>
                <div class="d-flex justify-content-start align-items-center check-content">
                  <input type="checkbox" class="check-filter" :checked="filters.availability.includes('outOfStock')"
                    @change="updateAvailabilityFilter('outOfStock', $event)" />
                  <p class="check-label">Agotado</p>
                </div>
              </div>
            </transition>
          </div>
          ...
          <div class="form-group filter-group2">
            <div class="d-flex justify-content-between align-items-center cursor-pointer"
              ...
              <label class="title-filter">TALLAS</label>
              <i class="fas fa-chevron-down" :class="{ 'rotated': showSections.sizes }"></i>
            </div>
            <transition name="fade">
              <div v-if="showSections.sizes" class="d-flex flex-wrap gap-2 mt-3">
                <button v-for="(item, index) in sizes" :key="index"
                  class="btn btn-outline-dark btn-sm px-3 py-1 rounded size-btn"
                  :class="{ 'selected': filters.sizes.includes(item.SizeName) }"
                  @click="updateSizeFilter(item.SizeName)">
                  {{ item.SizeName }}
                </button>
              </div>
            </transition>
          ...
          <div class="form-group filter-group2">
            <div class="d-flex justify-content-between align-items-center cursor-pointer"
              ...
              <label class="title-filter" for="">COLECCIONES</label>
              <i class="fas fa-chevron-down" :class="{ 'rotated': showSections.collections }"></i>
            </div>
            <transition name="fade">
              <div v-if="showSections.collections" class="mt-3">
                <div v-for="(item, index) in collections" :key="index"
                  class="d-flex justify-content-start align-items-center check-content">
                  <input type="checkbox" class="check-filter" :checked="filters.collections.includes(item.CollectionID)"
                    @change="updateCollectionFilter(item.CollectionID, $event)" />
                  <p class="check-label">{{ item.CollectionName }}</p>
                </div>
              </div>
            </transition>
          </div>
          ...
          <div class="form-group filter-group2">
            <div class="d-flex justify-content-between align-items-center cursor-pointer"
              ...
              <label class="title-filter" for="">COLOR</label>
              <i class="fas fa-chevron-down" :class="{ 'rotated': showSections.colors }"></i>
            </div>
            <transition name="fade">
              <div v-if="showSections.colors" class="mt-3">
                <div v-for="(item, index) in colors" :key="index"
                  class="d-flex justify-content-start align-items-center check-content">
                  <input type="checkbox" class="check-filter-color" :style="{ '--color-hex': item.HexCode }"
                    :checked="filters.colors.includes(item.ColorID)"
                    @change="updateColorFilter(item.ColorID, $event)" />
                  <p class="check-label">{{ item.ColorName }}</p>
                </div>
              </div>
            </transition>
          </div>
        ...
        <div class="col-lg-9 col-md-12">
          <div class="d-flex flex-wrap justify-content-around">
            <div class="row content-card-images">
              <div class="col-12 col-lg-3 col-md-4 card-images"

                v-for="(item, index) in filteredProducts.filter(item=>item.Tipo == 'Jeans Rat')" :key="index">
                <a :href="'/detail_novias?product_id='+item.ProductID">
                  <img :src="item.images[0]?.ImagePath" alt="Imagen del producto" class="image-product" loading="lazy">
                </a>
                <div class="d-flex justify-content-between image-product-details align-items-center">
                  <div class="d-flex flex-wrap gap-2">
                    <button v-for="(item, index) in item.colors.slice(-3)" :key="index"
                      class="btn btn-sm rounded btn-color-card"
                      :style="{'--color-card-hex': item.HexCode}"></button>
                  </div>
                  <div class="heart-checkbox d-flex align-items-center">
                    <input type="checkbox" :id="'heart-' + item.ProductID" :checked="item.isFavorite"
                      @click="updateFavorites(item)" class="form-check-input" style="opacity: 0; position: absolute;" />
                    <label :for="'heart-' + item.ProductID" class="heart card-heart"
                      :class="{ 'active': item.isFavorite }" @click.prevent="handleFavoriteClick(item)"></label>
                  </div>
                </div>
                <div class="image-product-details text-start">
                  <p class="product-name">{{item.Name}}</p>
                  <p class="product-price">S/{{item.Price}}</p>
                </div>
              </div>
            </div>
          </div>
  ...
  <footer class="text-center">
    <div class="row footer-section-contact">
      ...
      <!-- <div class="col-md-4 section-middle">
        ...
        <small>
          ...
          de Perlux. Consulta nuestros <a href="#">Términos y Condiciones</a> | <a href="#">Política de
            Privacidad</a>.
        </small>
      </div> -->
      <div class="col-md-4 section-middle">
    ...
    <a class="whatsapp-float" href="https://wa.me/51920775278" target="_blank">
      <img src="images/site/whatsapp.webp" alt="WhatsApp" class="float-icon" loading="lazy">
    </a>

    <!-- Botón flecha hacia arriba -->
    <a class="arrow-top" href="#top">
      <img src="images/site/svg/arrow-top.svg" alt="Ir arriba" class="float-icon" loading="lazy">
    </a>
  </footer>

  <!-- Modal: Inicia sesión o regístrate para usar favoritos -->
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body text-center p-4">
          <div class="mb-3">
            <i class="bx bx-heart" style="font-size: 48px; color: #ff6b9d;"></i>
          </div>
          <h5 class="mb-3">¿Quieres guardar este producto en tus favoritos?</h5>
          <p class="text-muted">
            Inicia sesión o crea una cuenta para guardar tus productos favoritos y acceder desde cualquier dispositivo.
          </p>

          <div class="d-grid gap-3 mt-4">
            <a class="btn btn-dark btn-lg" href="/register">
              Regístrate
            </a>
            <a class="btn btn-outline-dark btn-lg" href="/user">
              Inicia sesión con tu cuenta
            </a>
            <button class="btn btn-link text-secondary" @click="closeAuthModal">
              Entendido
            </button>
          </div>
        </div>
...
export default {
  ...
  data() {
    ...
    computed: {
      ...
      filteredProducts() {
        ...
        // 1. Filtro por Colección (si está seleccionada)
        // if (this.collection !='' && this.collection !== null && this.collection !== undefined) {
        //   filtered = filtered.filter(product => 
        //     product.CollectionID == this.collection
        //   );
        // }
        ...
        if (this.filters.colors.length > 0) {
          filtered = filtered.filter((product) =>
            product.colors.some((color) =>
              this.filters.colors.includes(color.ColorID)
            )
          );
        }
        ...
        if (this.filters.sizes.length > 0) {
          filtered = filtered.filter((product) =>
            product.sizes.some((size) =>
              this.filters.sizes.includes(size.SizeName)
            )
          );
        }
    ...
    methods: {
          ...
          switch (filterValue) {
            ...
            case '7': // Destacado
              this.filteredProducts.sort((a, b) => {
                if (a.IsFeatured === 1 && b.IsFeatured === 0) {
                  return -1; // `a` va primero
                }
                if (a.IsFeatured === 0 && b.IsFeatured === 1) {
        ...
        //     this.favorites.push(product.id); // Add to favorites if checked
        //   } else {
        //     this.favorites = this.favorites.filter(id => id !== product.id); // Remove if unchecked
        //   }
  ...
  async mounted() {
    ...
    await this.loadData();
    // this.favorites = JSON.parse(localStorage.getItem('favorites')) || []; // Load from localStorage
    // this.products.forEach(product => {
    //   product.isFavorite = this.favorites.includes(product.id); // Sync initial state
    // });
...
@media (max-width: 991.98px) {
  ...
  .fourth-section-filters .form-group{
    margin-left:10px;
    margin-right:10px;
  }
